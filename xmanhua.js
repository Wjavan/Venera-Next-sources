...
class XManhua extends ComicSource {
  ...
  version = "1.0.0";
  minAppVersion = "1.6.0";
  url = "https://cdn.jsdelivr.net/gh/Wjavan/venera-next-sources@main/xmanhua.js";
  ...
  get baseUrl() {
    return this.loadSetting("domains") || "https://xmanhua.com";
  }
  ...
  explore = [{
      ...
      load: async (page) => {
        ...
        const result = {};
        let items = doc.querySelectorAll(".manga-1-list-item, .manga-i-list-item, .comic-list li, .book-list li, .comic-item, .list-item, .recommend-item, .comics li, article");
        if (items.length > 0) {
  ...
  search = {
      load: async (keyword, options, page) => {
        ...
        let doc = new HtmlDocument(res.body);
        let items = doc.querySelectorAll(".manga-1-list-item, .manga-i-list-item, .search-result li, .book-list li, .comic-item, .comics li, article");
        return {
  ...
  comic = {
      loadInfo: async (id) => {
        ...
        let title = doc.querySelector(".manga-1-list-title, .manga-i-list-title, .book-title, h1, .detail-title, .comic-title")?.text?.trim() || id;
        let cover = doc.querySelector(".manga-1-cover img, .manga-i-cover img, .book-cover img, .detail-cover img, .comic-cover img")?.attributes?.src || "";
        let author = "";
        let description = doc.querySelector(".book-intro, .intro, .description, .comic-desc")?.text?.trim() || "";
        let update = doc.querySelector(".last-chapter, .update-chapter, .latest-chapter, .manga-1-list-subtitle")?.text?.trim() || "";
      ...
      loadEp: async (comicId, epId) => {
        ...
        let images = [];
        let imgNodes = doc.querySelectorAll("#chapter-images img, .chapter-content img, .read-content img, .comic-page img, .manga-1-cover img, .manga-i-cover img");
