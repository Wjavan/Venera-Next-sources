...
class GoDaMH extends ComicSource {
    ...
    version = "1.0.0";
    minAppVersion = "1.6.0";
    url = "https://cdn.jsdelivr.net/gh/yourname/venera-sources@main/godamh.js";

    get baseUrl() { return "https://godamh.com"; }
    ...
    parseComic(e) {
        ...
        try {
            ...
            let authorEl = e.querySelector(".author");
    ...
    search = {
        load: async (keyword, options, page) => {
            ...
            if (res.status !== 200) url = `${this.baseUrl}/search?keyword=${encodeURIComponent(keyword)}&page=${page}`;
    ...
    explore = [{ title: "高达漫画", type: "singlePageWithMultiPart", load: async () => {
        ...
        for (let list of doc.querySelectorAll(".comic-list, .recommend-list, .list")) {
            let tEl = list.querySelector("h3, h2, .title");
        ...
        if (Object.keys(parts).length === 0) {
            let cs = []; for (let i of doc.querySelectorAll(".comic-item, .list-item")) { let c = this.parseComic(i); if (c.title) cs.push(c); }
            if (cs.length > 0) parts["推荐"] = cs;
        }
        return parts;
    }}];

    category = { title: "高达漫画分类", parts: [{ name: "全部", type: "fixed", categories: [
    ...
    comic = {
        ...
        link: { domains: ["godamh.com"], linkToId: (url) => { let m = url.match(/\/comic\/([\d\w-]+)/) || url.match(/\/([\d\w-]+)(?:_\d+)?\.?html?$/); return m ? m[1] : null; } },
        idMatch: "^(\\d+|[\\w-]+)$",
