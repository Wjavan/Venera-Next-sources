/**
 * 福漫画 - VeneraNext JS 图源
 * 站点: http://mh.fumanhua.net
 * 验证: 2026-09-19 HTTP 200
 */
class FuManhua extends ComicSource {
    name = "福漫画"; key = "fumh"; version = "1.0.0"; minAppVersion = "1.6.0";
    url = "https://cdn.jsdelivr.net/gh/yourname/venera-sources@main/fumh.js";
    get baseUrl() { return "http://mh.fumanhua.net"; }
    ...
    explore = [{ title: "福漫画", type: "singlePageWithMultiPart", load: async () => {
    ...
    category = { title: "福漫画分类", parts: [{ name: "全部", type: "fixed", categories: [
    ...
    comic = {
        ...
        link: { domains: ["mh.fumanhua.net"], linkToId: (url) => { let m = url.match(/\/comic\/([\d\w-]+)/) || url.match(/\/([\d\w-]+)(?:_\d+)?\.?html?$/); return m ? m[1] : null; } },
