...
class Manhuaguitw extends ComicSource {
  name = "漫画柜繁体";
  key = "manhuaguitw";
  version = "1.0.0";
  minAppVersion = "1.6.0";
  url = "https://cdn.jsdelivr.net/gh/Wjavan/venera-next-sources@main/manhuaguitw.js";
  ...
  settings = {
    domains: {
      ...
      type: "select",
      options: [
        { value: "https://tw.manhuagui.com", text: "https://tw.manhuagui.com (主站)" }
      ],
      default: "https://tw.manhuagui.com"
    }
  };

  get baseUrl() {
    return this.loadSetting("domains") || "https://tw.manhuagui.com";
  }
  ...
  parseComic(item) {
    ...
    const cover = item.querySelector("img")?.attributes?.src || item.querySelector("img")?.attributes?.src;
    const author = item.querySelector(".author, .book-author")?.text?.trim() || "";
    const update = item.querySelector(".update, .chapter")?.text?.trim() || "";
