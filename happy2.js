...
class Happy2 extends ComicSource {
  name = "嗨皮漫画";
  key = "happy2";
  version = "1.0.0";
  minAppVersion = "1.6.0";
  url = "https://cdn.jsdelivr.net/gh/Wjavan/venera-next-sources@main/happy2.js";
  ...
  get baseUrl() {
    return this.loadSetting("domains") || "https://www.laimanhua88.com";
  }
  ...
  parseComic(item) {
    ...
    const title = item.querySelector(".manga-1-list-title, .title, h3")?.text?.trim() || link.text.trim();
    const cover = item.querySelector(".manga-1-cover img, img")?.attributes?.src || item.querySelector("img")?.attributes?.src;
    const author = item.querySelector("")?.text?.trim() || "";
    const update = item.querySelector(".manga-1-list-subtitle, .update")?.text?.trim() || "";
  ...
  explore = [{
    ...
    load: async (page) => {
      ...
      const result = {};
      let items = doc.querySelectorAll(".manga-1-list-item, .manga-i-list-item, .comic-item");
      if (items.length > 0) {
  ...
  comic = {
    loadInfo: async (id) => {
      ...
      let cover = doc.querySelector(".manga-1-cover img, .book-cover img")?.attributes?.src || "";
      let author = doc.querySelector(".author, .book-author")?.text?.trim() || "";
      ...
      let chapterLinks = doc.querySelectorAll(".chapter-list a, .book-chapters a, .catalog a");
      let i = 0;
