...
class Komiic extends ComicSource {
  ...
  version = "1.0.0";
  minAppVersion = "1.6.0";
  url = "https://cdn.jsdelivr.net/gh/Wjavan/venera-next-sources@main/komiic.js";
  ...
  get baseUrl() {
    return this.loadSetting("domains") || "https://komiic.com";
  }
  ...
  explore = [{
    ...
    load: async (page) => {
      ...
      const result = {};
      let items = doc.querySelectorAll(".comic-list li, .book-list li, .comic-item, .list-item, article");
      if (items.length > 0) {
  ...
  search = {
    load: async (keyword, options, page) => {
      let url = `${this.baseUrl}/search/${encodeURIComponent(keyword)}/page/${page}/`;
      let res = await Network.get(url, this.headers);
      if (res.status !== 200) throw `搜索失败: ${res.status}`;
      let doc = new HtmlDocument(res.body);
      let items = doc.querySelectorAll(".search-result li, .book-list li, .comic-item, article");
      return {
