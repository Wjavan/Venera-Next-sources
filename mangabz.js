...
class Mangabz extends ComicSource {
  ...
  explore = [{
    ...
    load: async (page) => {
      ...
      const result = {};
      let items = doc.querySelectorAll(".comic-list li, .book-list li, .comic-item, .list-item, .comics li");
      if (items.length > 0) {
  ...
  search = {
    load: async (keyword, options, page) => {
      ...
      let doc = new HtmlDocument(res.body);
      let items = doc.querySelectorAll(".search-result li, .book-list li, .comic-item, .comics li");
      return {
