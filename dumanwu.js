/** 毒漫画 DUMANWU - http://dumanwu1.com - HTTP 200 */
class Dumanwu extends ComicSource {
    name = "毒漫画"; key = "dumanwu"; version = "1.0.0"; minAppVersion = "1.6.0";
    url = "https://cdn.jsdelivr.net/gh/yourname/venera-sources@main/dumanwu.js";
    get baseUrl() { return "http://dumanwu1.com"; }
    ...
    parseComic(e) {
        ...
        try { let a = e.querySelector("a"); href = a ? a.attributes["href"] : "";
            title = e.querySelector("h3, h4, .title") ? (e.querySelector("h3, h4, .title").text.trim()) : (a ? a.text.trim() : "");
            let img = e.querySelector("img, mip-img"); if (img) cover = img.attributes["src"] || img.attributes["data-src"] || "";
            let aEl = e.querySelector(".author"); author = aEl ? aEl.text.trim() : "";
    ...
    search = { load: async (kw, opts, page) => {
        let res = await Network.get(`${this.baseUrl}/search?keyword=${encodeURIComponent(kw)}&page=${page}`, this.headers);
        if (res.status !== 200) throw `搜索失败: HTTP ${res.status}`;
        let doc = new HtmlDocument(res.body); let cs = [];
        for (let i of doc.querySelectorAll(".comic-item, .search-item, .list-item, .comic-list li")) { let c = this.parseComic(i); if (c.title && c.id) cs.push(c); }
        return { comics: cs, maxPage: cs.length > 0 ? page + 1 : page };
    }, optionList: [] };
    explore = [{ title: "毒漫画", type: "singlePageWithMultiPart", load: async () => {
        ...
        for (let l of doc.querySelectorAll(".comic-list, .recommend-list, .list")) {
            let tEl = l.querySelector("h3, h2, .title"); let pt = tEl ? tEl.text.trim() : "推荐";
            let cs = []; for (let i of l.querySelectorAll("li")) { let c = this.parseComic(i); if (c.title) cs.push(c); }
            if (cs.length > 0) parts[pt] = cs;
        }
        if (Object.keys(parts).length === 0) { let cs = []; for (let i of doc.querySelectorAll(".comic-item, .list-item")) { let c = this.parseComic(i); if (c.title) cs.push(c); } if (cs.length > 0) parts["推荐"] = cs; }
        return parts;
    }}];
    category = { title: "毒漫画分类", parts: [{ name: "全部", type: "fixed", categories: [
    ...
    categoryComics = { load: async (cat, param, opts, page) => {
        ...
        let doc = new HtmlDocument(res.body); let cs = [];
        for (let i of doc.querySelectorAll(".comic-item, .list-item, .comic-list li")) { let c = this.parseComic(i); if (c.title) cs.push(c); }
    ...
    comic = {
        loadInfo: async (id) => {
            ...
            let title = doc.querySelector("h1") ? (doc.querySelector("h1").text.trim()) : "";
            let cover = "", coverEl = doc.querySelector("img.cover, .book-cover img");
            if (coverEl) cover = this.fixUrl(coverEl.attributes["src"] || coverEl.attributes["data-src"] || "");
            let intro = doc.querySelector(".intro, .description") ? (doc.querySelector(".intro, .description").text.trim()) : "";
            let author = doc.querySelector(".author") ? (doc.querySelector(".author").text.trim()) : "";
            ...
            for (let sel of ["ul.catalog-list > li > a", "#chapter-list li a"]) {
        ...
        loadEp: async (comicId, epId) => {
            ...
            if (images.length === 0) { let doc = new HtmlDocument(html); for (let img of doc.querySelectorAll(".reader-area img, .chapter-img, #img-list img")) { let src = img.attributes["data-src"] || img.attributes["data-original"] || img.attributes["src"] || ""; if (src && !src.includes("data:image")) images.push(this.fixUrl(src)); } }
        ...
        link: { domains: ["dumanwu1.com"], linkToId: (url) => { let m = url.match(/\/comic\/([\d\w-]+)/) || url.match(/\/([\d\w-]+)(?:_\d+)?\.?html?$/); return m ? m[1] : null; } },
