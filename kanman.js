...
class Kanman extends ComicSource {
  ...
  parseComic(item) {
    ...
    const title = item.querySelector(".title, h3, .book-title")?.text?.trim() || link.text.trim();
  ...
  explore = [{
    ...
    load: async (page) => {
      ...
      if (items.length > 0) {
        result["热门推荐"] = items.map(e => this.parseComic(e)).filter(Boolean);
      }
      return result;
    }
  }];
  ...
  categoryComics = {
    load: async (category, param, options, page) => {
      ...
      let doc = new HtmlDocument(res.body);
      let items = doc.querySelectorAll(".comic-list li, .book-list li, .comic-item");
      return {
